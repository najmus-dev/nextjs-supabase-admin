...
import { deleteBusinessCategory } from "@/services/categories_services";
...
export function DataTableRowActions<BusinessCategory>({
  row,
}: DataTableRowActionsProps<BusinessCategory>) {
  const client = createClient();
  const queryClient = useQueryClient();

  const handleDeleteCategory = async (id: string) => {
    toast.promise(deleteBusinessCategory(client, id), {
      loading: "Loading...",
      success: async (_) => {
      ...
      <DropdownMenuContent align="end" className="w-[160px]">
        ...
        <AddCategorySheet
          category={{
            ...
            is_featured: row.getValue("is_featured"),
